...
bool Timer_AddTicks(struct Console* sys, struct Timer* timers, const int timernum, timestamp ticks, bool a9)
{
    ...
    if (ticks >= remaining) // overflow occured; reload needed.
    {
        ...
        if ((timernum != 3) // not the last timer
            ...
            && (timers[timernum+1].Control.Enable || timers[timernum+1].NeedsPowerOff)) // next timer is running
        {
...
void Timer_UpdateCR(struct Timer* timer)
{
    ...
    else if (oldenable && !timer->Control.Enable)
    {
        timer->NeedsPowerOff = true;
    }
...
void Timer_Run(struct Console* sys, struct Timer* timers, const timestamp until, bool a9)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        timer->LastUpdated += 1;
        if (!timer->Control.OverflowTick && (timer->Control.Enable || timer->NeedsPowerOff)) // timer not running
        {
            ...
            overflowed = Timer_AddTicks(sys, timers, i, ticks, a9);
        }
        ...
        if (timer->NeedsPowerOff)
        {
            timer->BuggyOverflow = overflowed;
            if ((timer->LastUpdated >= until)) continue;
            if (timer->BuggyOverflow
                && (i != 3) // not the last timer
                && timers[i+1].Control.OverflowTick // next timer ticks when we overflow
                && (timers[i+1].Control.Enable || timers[i+1].NeedsPowerOff)) // next timer is running
            {
                //timers[i+1].LastUpdated += 1; // checkme?
                Timer_AddTicks(sys, timers, i+1, 1, a9); // tick the next timer
            }
        }
    }
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        if (!(timer->Control.Enable || timer->NeedsPowerOff)) { timer->LastUpdated = until; continue; } // timer not running

        // this divider behavior probably needs more verification?
        timestamp ticks = (until >> timer->DividerShift) - (timer->LastUpdated >> timer->DividerShift);

        timer->LastUpdated = until; 
        Timer_AddTicks(sys, timers, i, ticks, a9);
    }
